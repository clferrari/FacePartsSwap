    ...
    if np.in1d('FACE', parts_to_swap_upper):
        ...
        for part in parts_to_swap_upper:
            if part == 'MOUTH':
                ...
                eyes_indexes = np.concatenate((left_eye_indexes, right_eye_indexes), axis=1)
                eyes_indexes -= 1
                indexes_to_swap.extend(get_inside_index(eyes_indexes.squeeze(), projected_mesh_src, idxm2))
            elif part == 'EYEBROWS':
                eyebrows_indexes = []
                ...
                eyebrows_indexes.extend(left_eyebrows_indexes)
                eyebrows_indexes.extend(right_eyebrows_indexes)
                indexes_to_swap.extend(get_inside_index(eyebrows_indexes, projected_mesh_src, idxm2))
            elif part == 'LEFT_EYE':
                left_eye_indexes = np.transpose(np.array(face_indices_contour["leye"])).astype(int)
                left_eye_indexes -= 1
                indexes_to_swap.extend(get_inside_index(left_eye_indexes, projected_mesh_src, idxm2))
            elif part == 'RIGHT_EYE':
                right_eye_indexes = np.transpose(np.array(face_indices_contour["reye"])).astype(int)
                right_eye_indexes -= 1
                indexes_to_swap.extend(get_inside_index(right_eye_indexes, projected_mesh_src, idxm2))
            elif part == 'LEFT_EYEBROW':
                left_eyebrows_indexes = np.load('3D_data/index/leyebrowIndex.npy.npy')
                indexes_to_swap.extend(get_inside_index(left_eyebrows_indexes, projected_mesh_src, idxm2))
            elif part == 'RIGHT_EYEBROW':
                right_eyebrows_indexes = np.load('3D_data/index/reyebrowIndex.npy.npy')
                indexes_to_swap.extend(get_inside_index(right_eyebrows_indexes, projected_mesh_src, idxm2))

    indexes_to_swap = list(dict.fromkeys(indexes_to_swap))
